...
class TestAssetEndpoints:
	...
	def test_for_invalid_category_id(self, client, init_db, auth_header):
		...
		assert response_json['status_code'] == 400
		assert response_json['status'] == 'error'
		assert response_json['message'] == 'Invalid asset category id'

	def test_for_missing_tag(self, client, init_db, auth_header, new_asset_category):
	...
	def test_for_missing_serial(self, client, init_db, auth_header, new_asset_category):
		new_asset_category.save()
		...
		assert response_json['status'] == 'error'
		assert response_json['message'] == 'An error occurred'
		assert response_json['errors'] == {'serial': ['This field is required']}

	def test_for_valid_data(self, client, init_db, auth_header, new_asset_category):
		new_asset_category.save()
